...
class CUserCart {
	...
	public function changeCountInGoods() {
		//получить корзину(это $this->cart)
		//кол-во товара в корзине в "count_in_cart"
		//в цикле обойти корзину и уменьшить кол-во товара
		foreach ($this->cart as $key => $value) {
			//получить кол-во товара в базе
			$count_in_base = $this->getGoodsCount($value['id']);
			// echo "кол-во товара в базе goods: ". $count_in_base."<br>";
			// echo "кол-во товара в корзине: ". $value['count_in_cart']."<br>";
			$new_count = $count_in_base - $value['count_in_cart'];
			if($new_count >= 0) { //на всякий случай
