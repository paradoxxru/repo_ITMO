    ...
    private function getValue($val, $level)
    {
        if(empty($val)) {
            ...
            for($this->i++;$this->i < $this->total_lines; $this->i++)
            {
                ...
                // В двумерном массиве $matches будут: знаки табуляции
                
                if(!isset($matches[0][0]) || strlen($matches[0][0]) <= $level) {
                    //если не находим табуляции, то i "возвращаем на шаг назад"
                    //и возвращаем ассоц масств $val(то что зополнилось до этого возвращаем в ф-цию read())
                    $this->i--;
                    return $val;
                }
