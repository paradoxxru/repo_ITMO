...
class User
{
	...
	private static function registerUser($login, $password) { //добавляет в файл строчку с логином и паролем(разделяем ":"")
	...
	// для замены  registerUser($login, $password)
	public function registration($login, $password, $params) { // регистрация. в $params - будут содержаться разные значения(телефон, адресс, email и тд), которые могут добавляться или меняться
